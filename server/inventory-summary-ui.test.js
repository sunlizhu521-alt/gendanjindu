...
test('库存数据提供独立的底表文件和手工表库', () => {
  ...
  assert.match(client, /\^inventorySummaryFile1\[0-6\]\$\/\.test\(slot\.id\)[\s\S]*?\? '备用'/);
  assert.match(client, /slot\.id === 'inventorySummaryFile8' \? '不可售手工' : `\$\{slot\.title\}手工`/);
  assert.match(client, /requiredFields: \[\]/);
  ...
  assert.match(client, /请按需要手动选择原表字段，未选择的字段按空值处理，不会自动匹配/);
  ...
  assert.match(server, /strictMapping: isInventoryManualSlot\(slotId\),[\s\S]*?allowIncompleteMapping: isInventoryManualSlot\(slotId\)/);
