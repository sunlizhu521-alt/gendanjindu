...
function useFilteredDemands(rows) {
  const [filters, setFilters] = useState({ keyword: '', month: '', businessUnit: '', supplier: '', productLine: '', purchaseGroup: '' });
  const options = useMemo(() => {
    const unique = (key) => [...new Set(rows.map((row) => normalize(row[key])).filter(Boolean))].sort((a, b) => a.localeCompare(b, 'zh-Hans-CN'));
    return {
      months: unique('month'),
...
function FilterBar({ filters, setFilters, options }) {
  return (
    <div className="toolbar filters-row">
      ...
      <button type="button" className="ghost compact-button" onClick={() => setFilters({ keyword: '', month: '', businessUnit: '', supplier: '', productLine: '', purchaseGroup: '' })}>清空筛选</button>
    </div>
  );
}
