const INVENTORY_CALCULATION_SOURCES = [
  ...
  },
  {
    ...
    filters: '库存量(主单位)为0、空值或非法值不进入映射与汇总；仓库名称必须在仓库名称维度中唯一匹配且站点=中国；映射后保留国内事业部及销售部-工厂',
    ...
    <section className="inventory-methodology">
      ...
      <aside className="inventory-methodology-pending" aria-label="不可售库存口径">
        ...
        <div>
          ...
          <p>国内在库报表同时直接识别原始仓库编码555、777；FBM库存同时识别原始仓库名称中的退货仓；FBM在途以“收货仓库”为777开头识别不可售在途。</p>
      ...
      <section className="inventory-methodology-section">
        ...
        <div className="inventory-rule-grid">
          ...
          <article>
            <strong>缺失与冲突</strong>
            <p>维度缺失或同一键存在多个结果时标记映射冲突并进入诊断。国内在库先按仓库名称维度仅保留站点为“中国”的记录，再纳入明确映射为国内事业部或销售部-工厂的记录。</p>
          </article>
          <article>
            <strong>数量清洗</strong>
